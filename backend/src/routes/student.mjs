  ...
  try {
    ...
    const data = {
      ...
      userDetails: {
        ...
        lastName: user.lastName,
        email: user.email,
        isVerified: user.isVerified || false,  // ✅ Added verification status
        verifiedAt: user.verifiedAt || null   // ✅ Added verification timestamp (optional)
      }
    };
...
studentRouter.put("/details/:userID", async (req, res) => {
  const { userID } = req.params;
  const {
    firstName,
    lastName,
  ...
  try {
    ...
    if (profileImageBase64) {
      ...
      const profileImage = `profile_${Date.now()}.${imageType}`;
      await fs.writeFile(path.join(UPLOADS_DIR, profileImage), buffer);
...
studentRouter.get("/:userID/joined-classes/count", async (req, res) => {
  try {
    const student = await StudentUser.findOne({ userID: req.params.userID })
      .populate("userID", "joinedClasses"); // only get joinedClasses
    ...
    const count = student.userID.joinedClasses.length;

    res.status(200).json({
      success: true,
      joinedClassesCount: count
    });
    ...
    res.status(500).json({
      success: false,
      error: "Failed to fetch joined classes count",
      details: process.env.NODE_ENV === "development" ? err.message : undefined
    });
  }
});
...
studentRouter.get("/:userID/joined-classes/schedule-count", async (req, res) => {
  try {
    // 1. Find the student and get joined classes
    const student = await StudentUser.findOne({ userID: req.params.userID })
      .populate("userID", "joinedClasses");
    ...
    const joinedClasses = student.userID.joinedClasses;
    ...
    // 2. Count schedules from Timetable for all joined classes
    const scheduleCount = await Timetable.countDocuments({
      classId: { $in: joinedClasses }
    });
